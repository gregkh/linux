...
void Hacl_Streaming_Keccak_reset(Hacl_Streaming_Keccak_state *s)
{
	...
	Hacl_Streaming_Keccak_state tmp = { .block_state = block_state,
					    .buf = buf,
					    .total_len =
			     ...
			     uint32_t len)
{
	...
	if (len <= block_len(i) - sz) {
		...
		*p = ((Hacl_Streaming_Keccak_state){ .block_state =
							     block_state1,
						     .buf = buf,
...
static void finish_(Spec_Hash_Definitions_hash_alg a,
		    Hacl_Streaming_Keccak_state *p, uint8_t *dst, uint32_t l)
{
	...
	uint8_t *buf_1 = buf_;
	uint64_t buf[25U] = { 0U };
	Hacl_Streaming_Keccak_hash_buf tmp_block_state = { .fst = a,
							   .snd = buf };
...
Hacl_Streaming_Types_error_code
Hacl_Streaming_Keccak_squeeze(Hacl_Streaming_Keccak_state *s, uint8_t *dst,
			      uint32_t l)
{
	Spec_Hash_Definitions_hash_alg a1 = Hacl_Streaming_Keccak_get_alg(s);
	if (!(a1 == Spec_Hash_Definitions_Shake128 ||
