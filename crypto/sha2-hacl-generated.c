...
{
	KRML_MAYBE_FOR8(i, (uint32_t)0U, (uint32_t)8U, (uint32_t)1U,
			uint32_t *os = hash;
...
static inline void sha256_update_(uint8_t *b, uint32_t *hash)
{
	...
	ws[15U] = u14;
	KRML_MAYBE_FOR4(
		i0, (uint32_t)0U, (uint32_t)4U, (uint32_t)1U,
		KRML_MAYBE_FOR16(
			i, (uint32_t)0U, (uint32_t)16U, (uint32_t)1U,
			uint32_t k_t = Hacl_Impl_SHA2_Generic_k224_256
				[(uint32_t)16U * i0 + i];
			uint32_t ws_t = ws[i]; uint32_t a0 = hash[0U];
			uint32_t b0 = hash[1U]; uint32_t c0 = hash[2U];
			uint32_t d0 = hash[3U]; uint32_t e0 = hash[4U];
			uint32_t f0 = hash[5U]; uint32_t g0 = hash[6U];
			uint32_t h02 = hash[7U]; uint32_t k_e_t = k_t;
			uint32_t t1 =
				h02 +
			...
			uint32_t t2 =
				...
				((a0 & b0) ^ ((a0 & c0) ^ (b0 & c0)));
			uint32_t a1 = t1 + t2; uint32_t b1 = a0;
			uint32_t c1 = b0; uint32_t d1 = c0;
			uint32_t e1 = d0 + t1; uint32_t f1 = e0;
			uint32_t g1 = f0; uint32_t h12 = g0; hash[0U] = a1;
			hash[1U] = b1; hash[2U] = c1; hash[3U] = d1;
			hash[4U] = e1; hash[5U] = f1; hash[6U] = g1;
			hash[7U] = h12;);
		if (i0 < (uint32_t)3U) {
			KRML_MAYBE_FOR16(
				i, (uint32_t)0U, (uint32_t)16U, (uint32_t)1U,
				uint32_t t16 = ws[i];
				uint32_t t15 =
					ws[(i + (uint32_t)1U) % (uint32_t)16U];
				uint32_t t7 =
					ws[(i + (uint32_t)9U) % (uint32_t)16U];
				uint32_t t2 =
					ws[(i + (uint32_t)14U) % (uint32_t)16U];
				uint32_t s1 = (t2 << (uint32_t)15U |
					       t2 >> (uint32_t)17U) ^
					      ((t2 << (uint32_t)13U |
						t2 >> (uint32_t)19U) ^
					       t2 >> (uint32_t)10U);
				uint32_t s0 = (t15 << (uint32_t)25U |
					       t15 >> (uint32_t)7U) ^
					      ((t15 << (uint32_t)14U |
						t15 >> (uint32_t)18U) ^
					       t15 >> (uint32_t)3U);
	...
	KRML_MAYBE_FOR8(i, (uint32_t)0U, (uint32_t)8U, (uint32_t)1U,
			uint32_t *os = hash;
			uint32_t x = hash[i] + hash_old[i]; os[i] = x;);
}
					   ...
					   uint8_t *b, uint32_t *hash)
{
	...
	memcpy(last + fin - (uint32_t)8U, totlen_buf,
...
update_224_256(Hacl_Streaming_MD_state_32 *p, uint8_t *data, uint32_t len)
{
	...
	uint32_t sz;
	if (total_len % (uint64_t)(uint32_t)64U == (uint64_t)0U &&
	...
	if (len <= (uint32_t)64U - sz) {
		...
		uint32_t sz1;
		if (total_len1 % (uint64_t)(uint32_t)64U == (uint64_t)0U &&
		...
		*p = ((Hacl_Streaming_MD_state_32){ .block_state = block_state1,
						    .buf = buf,
		...
		if (!(sz1 == (uint32_t)0U)) {
			Hacl_SHA2_Scalar32_sha256_update_nblocks(
				(uint32_t)64U, buf, block_state1);
		}
		uint32_t ite;
		if ((uint64_t)len % (uint64_t)(uint32_t)64U == (uint64_t)0U &&
		...
		Hacl_SHA2_Scalar32_sha256_update_nblocks(
			data1_len / (uint32_t)64U * (uint32_t)64U, data1,
			block_state1);
		uint8_t *dst = buf;
		...
		*p = ((Hacl_Streaming_MD_state_32){
		...
		uint32_t sz10;
		if (total_len10 % (uint64_t)(uint32_t)64U == (uint64_t)0U &&
		    total_len10 > (uint64_t)0U) {
			sz10 = (uint32_t)64U;
		} else {
			sz10 = (uint32_t)(total_len10 %
					  (uint64_t)(uint32_t)64U);
		}
		uint8_t *buf2 = buf0 + sz10;
		memcpy(buf2, data1, diff * sizeof(uint8_t));
		uint64_t total_len2 = total_len10 + (uint64_t)diff;
		*p = ((Hacl_Streaming_MD_state_32){ .block_state =
							    block_state10,
						    .buf = buf0,
						    .total_len = total_len2 });
		...
		uint32_t ite;
		if ((uint64_t)(len - diff) % (uint64_t)(uint32_t)64U ==
			    (uint64_t)0U &&
		...
		Hacl_SHA2_Scalar32_sha256_update_nblocks(
			data1_len / (uint32_t)64U * (uint32_t)64U, data11,
			block_state1);
		uint8_t *dst = buf;
...
void Hacl_Streaming_SHA2_finish_256(Hacl_Streaming_MD_state_32 *p, uint8_t *dst)
{
	...
	Hacl_SHA2_Scalar32_sha256_update_nblocks((uint32_t)0U, buf_multi,
						 tmp_block_state);
	uint64_t prev_len_last = total_len - (uint64_t)r;
	Hacl_SHA2_Scalar32_sha256_update_last(prev_len_last + (uint64_t)r, r,
					      buf_last, tmp_block_state);
...
void Hacl_Streaming_SHA2_finish_224(Hacl_Streaming_MD_state_32 *p, uint8_t *dst)
{
	...
	uint64_t prev_len_last = total_len - (uint64_t)r;
	Hacl_SHA2_Scalar32_sha224_update_last(prev_len_last + (uint64_t)r, r,
					      buf_last, tmp_block_state);
...
static inline void sha512_update(uint8_t *b, uint64_t *hash)
{
	...
	ws[15U] = u14;
	KRML_MAYBE_FOR5(
		i0, (uint32_t)0U, (uint32_t)5U, (uint32_t)1U,
		KRML_MAYBE_FOR16(
			i, (uint32_t)0U, (uint32_t)16U, (uint32_t)1U,
			uint64_t k_t = Hacl_Impl_SHA2_Generic_k384_512
				[(uint32_t)16U * i0 + i];
			uint64_t ws_t = ws[i]; uint64_t a0 = hash[0U];
			uint64_t b0 = hash[1U]; uint64_t c0 = hash[2U];
			uint64_t d0 = hash[3U]; uint64_t e0 = hash[4U];
			uint64_t f0 = hash[5U]; uint64_t g0 = hash[6U];
			uint64_t h02 = hash[7U]; uint64_t k_e_t = k_t;
			uint64_t t1 =
				h02 +
			...
			uint64_t t2 =
				...
				((a0 & b0) ^ ((a0 & c0) ^ (b0 & c0)));
			uint64_t a1 = t1 + t2; uint64_t b1 = a0;
			uint64_t c1 = b0; uint64_t d1 = c0;
			uint64_t e1 = d0 + t1; uint64_t f1 = e0;
			uint64_t g1 = f0; uint64_t h12 = g0; hash[0U] = a1;
			hash[1U] = b1; hash[2U] = c1; hash[3U] = d1;
			hash[4U] = e1; hash[5U] = f1; hash[6U] = g1;
			hash[7U] = h12;);
		if (i0 < (uint32_t)4U) {
			KRML_MAYBE_FOR16(
				...
				uint64_t t2 =
					ws[(i + (uint32_t)14U) % (uint32_t)16U];
				uint64_t s1 = (t2 << (uint32_t)45U |
					       t2 >> (uint32_t)19U) ^
					      ((t2 << (uint32_t)3U |
						t2 >> (uint32_t)61U) ^
					       t2 >> (uint32_t)6U);
				uint64_t s0 = (t15 << (uint32_t)63U |
					       t15 >> (uint32_t)1U) ^
					      ((t15 << (uint32_t)56U |
						t15 >> (uint32_t)8U) ^
					       t15 >> (uint32_t)7U);
	...
	KRML_MAYBE_FOR8(i, (uint32_t)0U, (uint32_t)8U, (uint32_t)1U,
			uint64_t *os = hash;
			uint64_t x = hash[i] + hash_old[i]; os[i] = x;);
}
					   ...
					   uint64_t *hash)
{
	...
	FStar_UInt128_uint128 total_len_bits =
		FStar_UInt128_shift_left(totlen, (uint32_t)3U);
	store128_be(totlen_buf, total_len_bits);
	uint8_t *b0 = b;
	memcpy(last, b0, len * sizeof(uint8_t));
	last[len] = (uint8_t)0x80U;
	memcpy(last + fin - (uint32_t)16U, totlen_buf,
...
update_384_512(Hacl_Streaming_MD_state_64 *p, uint8_t *data, uint32_t len)
{
	...
	uint32_t sz;
	if (total_len % (uint64_t)(uint32_t)128U == (uint64_t)0U &&
	    total_len > (uint64_t)0U) {
		sz = (uint32_t)128U;
	} else {
	...
	if (len <= (uint32_t)128U - sz) {
		...
		uint64_t total_len1 = s1.total_len;
		uint32_t sz1;
		if (total_len1 % (uint64_t)(uint32_t)128U == (uint64_t)0U &&
		    total_len1 > (uint64_t)0U) {
			sz1 = (uint32_t)128U;
		} else {
		...
		memcpy(buf2, data, len * sizeof(uint8_t));
		uint64_t total_len2 = total_len1 + (uint64_t)len;
		*p = ((Hacl_Streaming_MD_state_64){ .block_state = block_state1,
						    .buf = buf,
						    .total_len = total_len2 });
	} else if (sz == (uint32_t)0U) {
		Hacl_Streaming_MD_state_64 s1 = *p;
		...
		if (!(sz1 == (uint32_t)0U)) {
			Hacl_SHA2_Scalar32_sha512_update_nblocks(
				(uint32_t)128U, buf, block_state1);
		}
		uint32_t ite;
		if ((uint64_t)len % (uint64_t)(uint32_t)128U == (uint64_t)0U &&
		    (uint64_t)len > (uint64_t)0U) {
			ite = (uint32_t)128U;
		} else {
			ite = (uint32_t)((uint64_t)len %
		...
		uint8_t *data2 = data + data1_len;
		Hacl_SHA2_Scalar32_sha512_update_nblocks(
			data1_len / (uint32_t)128U * (uint32_t)128U, data1,
			block_state1);
		uint8_t *dst = buf;
		memcpy(dst, data2, data2_len * sizeof(uint8_t));
		*p = ((Hacl_Streaming_MD_state_64){
		...
		uint64_t total_len10 = s1.total_len;
		uint32_t sz10;
		if (total_len10 % (uint64_t)(uint32_t)128U == (uint64_t)0U &&
		    total_len10 > (uint64_t)0U) {
			sz10 = (uint32_t)128U;
		} else {
			sz10 = (uint32_t)(total_len10 %
		...
		memcpy(buf2, data1, diff * sizeof(uint8_t));
		uint64_t total_len2 = total_len10 + (uint64_t)diff;
		*p = ((Hacl_Streaming_MD_state_64){ .block_state =
							    block_state10,
						    .buf = buf0,
						    .total_len = total_len2 });
		...
		uint32_t ite;
		if ((uint64_t)(len - diff) % (uint64_t)(uint32_t)128U ==
			    (uint64_t)0U &&
		    (uint64_t)(len - diff) > (uint64_t)0U) {
			ite = (uint32_t)128U;
		} else {
		...
		uint8_t *data21 = data2 + data1_len;
		Hacl_SHA2_Scalar32_sha512_update_nblocks(
			data1_len / (uint32_t)128U * (uint32_t)128U, data11,
			block_state1);
		uint8_t *dst = buf;
		memcpy(dst, data21, data2_len * sizeof(uint8_t));
		*p = ((Hacl_Streaming_MD_state_64){
...
void Hacl_Streaming_SHA2_finish_512(Hacl_Streaming_MD_state_64 *p, uint8_t *dst)
{
	...
	uint8_t *buf_last = buf_1 + r - ite;
	uint8_t *buf_multi = buf_1;
	Hacl_SHA2_Scalar32_sha512_update_nblocks((uint32_t)0U, buf_multi,
						 tmp_block_state);
	uint64_t prev_len_last = total_len - (uint64_t)r;
...
void Hacl_Streaming_SHA2_finish_384(Hacl_Streaming_MD_state_64 *p, uint8_t *dst)
{
	...
	uint8_t *buf_last = buf_1 + r - ite;
	uint8_t *buf_multi = buf_1;
	Hacl_SHA2_Scalar32_sha384_update_nblocks((uint32_t)0U, buf_multi,
						 tmp_block_state);
	uint64_t prev_len_last = total_len - (uint64_t)r;
